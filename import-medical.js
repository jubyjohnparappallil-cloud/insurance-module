/**
 * ╔══════════════════════════════════════════════════════╗
 * ║   IMPORT — Shanthi Medical Center patients           ║
 * ║   Output: clinic-data.json                           ║
 * ╚══════════════════════════════════════════════════════╝
 *
 * Usage:
 *   node import-medical.js            ← run import
 *   node import-medical.js --dry-run  ← preview only, nothing saved
 */
...
// ── Config ────────────────────────────────────────────────────────
const BRANCH_MATCH = 'shanthi medical center';
...
function main() {
  console.log('\n╔══════════════════════════════════════════════════════╗');
  console.log('║   IMPORT: Shanthi Medical Center → clinic-data.json  ║');
  console.log('╚══════════════════════════════════════════════════════╝');
  ...
  let db = {
    ...
    nextIds: { patient: 1, consultation: 1, claim: 918, insurance: 1 }
  };
  if (fs.existsSync(DB_FILE)) {
    try { db = JSON.parse(fs.readFileSync(DB_FILE, 'utf8')); }
    catch(e) { console.error('⚠️  Cannot read clinic-data.json:', e.message); process.exit(1); }
  }
  ...
  for (const row of dataRows) {
    // ── Branch filter: ONLY Medical Center ──
  ...
  // ── Summary ──
  console.log('\n📊  Results:');
  console.log(`    ✅  Medical Center patients imported : ${added}`);
  console.log(`    ⏭️   Duplicate MR skipped             : ${skippedDup}`);
  console.log(`    🏢  Wellness branch skipped           : ${skippedBranch}`);
  ...
  if (added === 0) {
    console.log('\n⚠️  Nothing to import. clinic-data.json unchanged.');
    return;
  }
  ...
  console.log(`\n✅  Saved ${added} Medical Center patients to clinic-data.json`);
  console.log(`    Total patients in file: ${db.patients.length}`);
  console.log('\n▶️   Start Medical Center server:  node server.js\n');
}
